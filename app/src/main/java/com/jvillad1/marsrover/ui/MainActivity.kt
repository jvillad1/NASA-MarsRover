        ...
        Column(
            ...
            NavHost(navController, startDestination = "Rovers") {
                composable(
                    ...
                    RoversScreen(navController,roverUiState.value)
                }
                composable(
                    route = "RoverDetails",
                ) {
                    val rover = navController.previousBackStackEntry?.arguments?.getParcelable<RoverDetailUI>("rover")
                    rover?.let { roverDetailUI -> RoverDetailsScreen(roverDetailUI) }
                }
            }
        }
    }
